...
env.monitor.start('/tmp/invertedPendulum-experiment-1',force=True)
...
observation, reward, done, _ = env.step(np.array([2]))
...
for bi in xrange(big_its):
	...
	for i in xrange(small_its):
		...
		while True:
			...
			action=np.dot(param,np.hstack((1,old_action,old_observation,observation)))
			old_action=action
